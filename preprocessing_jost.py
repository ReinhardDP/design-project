...
adata.var_names = pd.read_csv(input_genes, delimiter = '\t', header=None)[1]
adata.obs_names = pd.read_csv(input_cells, delimiter = '\t', header=None)[0]
...
def simplify_variant_name(variant):    
    ...
    if len(parts) >= 2:
        if parts[0][0:3] == "neg": return f"ctrl_{parts[-1]}"
        return f"{parts[0]}_{parts[-1]}"
    else:
        return variant
variant2cell['variant'] = variant2cell['variant'].apply(simplify_variant_name)
